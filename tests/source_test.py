...
def test_generator_api() -> None:
    ...
    for task in tasks_gen: assert isinstance(task, task_ref.Task)
    for task in tasks_api: assert isinstance(task, task_ref.Task)
...
def test_file() -> None:
    '''
    На вход идет файл с неверным форматом и верным.
    '''
    ...
    for task in tasks: assert isinstance(task, task_ref.Task)